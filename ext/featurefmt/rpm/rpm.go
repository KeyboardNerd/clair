...
func (l lister) ListFeatures(files tarutil.FilesMap) (database.FeatureDetectResult, error) {
	...
	if !hasFile {
		return database.FeatureDetectResult{Status: database.NotFound}, nil
	}
	...
	if err != nil {
		...
		return database.FeatureDetectResult{}, commonerr.ErrFilesystem
	}
	...
	return database.FeatureDetectResult{
		...
		Features: database.ConvertFeatureSetToFeatures(packages)}, nil
}
